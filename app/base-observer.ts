...
const APP = {
  name: "Base Signal (Built for Base)",
...
async function readNetworkDetails(address: string) {
  const c = client();
...
async function readBlockDetails() {
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain validation, and real-time read-only blockchain signals.";
  ...
  btnBlock.textContent = "Block Details";
  ...
  btnConnect.onclick = async () => {
    try {
      ...
      const snap = await readNetworkDetails(session.address);
  ...
  btnBlock.onclick = async () => {
    try {
      render(["Fetching block details…"]);
      const b = await readBlockDetails();
      render([
        "Block Details",
        `Network: ${active.label}`,
